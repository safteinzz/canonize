//! `canon self`: manage the installed binary itself.
//!
//! `self update` shells out to `cargo install canonize --force`, the crate name even when the command you type is shorter.
...
fn update(args: UpdateArgs) {
    ...
    println!(
        "{} {}\n",
        "Updating canonize via".dimmed(),
        "cargo install canonize --force".bold()
    );
        ...
        .status()
    {
        ...
        Err(e) => {
            eprintln!("{} {e}", "canon: could not run cargo:".red());
            eprintln!(
                "{}",
...
fn check() {
    ...
    match latest() {
        Ok(latest) if newer(&latest, current) => {
            println!(
                "{} {} {}",
                format!("canonize {latest}").bold(),
                "is available, you have".dimmed(),
                current.bold()
            );
            println!("{} {}", "run".dimmed(), "canon self update".bold());
        }
        Ok(_) => println!(
            "{} {}",
            format!("canonize {current}").bold(),
            "is the latest release.".dimmed()
        ),
        Err(e) => {
            eprintln!("{} {e}", "canon: could not reach crates.io:".red());
            std::process::exit(1);
        }
    }
}

/// `cargo search` prints `canonize = "X.Y.Z"    # description` for an exact name
...
fn confirm() -> bool {
    print!(
        "{} {} ",
        "Update canonize to the latest release via cargo?".bold(),
        "[y/N]".dimmed()
    );
    io::stdout().flush().ok();
